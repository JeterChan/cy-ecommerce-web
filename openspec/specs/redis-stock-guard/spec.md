# redis-stock-guard Specification

## Purpose
Redis 預扣庫存機制，使用 DECRBY 原子操作在 DB 事務之前過濾庫存不足的請求，大幅降低高併發場景的回應時間。
...
**Note**: Lazy-init 路徑（DELETE → SET → DECRBY）在併發場景下存在競爭條件：多個請求可能同時偵測到 key 不存在並各自從 DB 載入、SET、DECRBY，導致 Redis 庫存值短暫不準確。此競爭條件屬於可接受風險，因為：
1. 僅在 Redis key 不存在時觸發（冷啟動或快取清除），正常運行中極少發生
2. DB 層的悲觀鎖（`SELECT ... FOR UPDATE`）是防止超賣的最終保障，Redis 僅為快速過濾層
3. 即使 Redis 暫時放行多餘請求，DB 事務仍會正確拒絕庫存不足的訂單
...
**Note**: `sync_stock` 用於 Admin 庫存調整時同步 Redis（`INCRBY`），屬於進階操作介面。