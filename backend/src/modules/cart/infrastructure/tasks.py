...
async def sync_member_cart_logic(user_id: str, task_instance=None):
    ...
    # 2. 分散式鎖，確保單一使用者同步串行化
    async with redis_manager.distributed_lock(f"cart_sync:{user_id}", expire=30) as acquired:
        if not acquired:
        ...
        async with sessionLocal() as session:
            ...
            if redis_items:
                ...
                upsert_stmt = upsert_stmt.on_conflict_do_update(
                    ...
                    set_={"quantity": upsert_stmt.excluded.quantity, 
                ...
                # Redis 為空，清空資料庫
                cleanup_stmt = delete(CartItemModel).where(CartItemModel.cart_id == cart_id)
                await session.execute(cleanup_stmt)
