...
@router.get("", summary="獲取商品列表 (管理員，支援搜尋/篩選/排序/分頁)")
async def admin_list_products(
    ...
    sort: str = Query(
        default="created_desc", description="排序：created_desc 或 created_asc"
    ),
    ...
    return {
        "products": [
            ProductResponseDTO.model_validate(p).model_dump() for p in products
        ],
        "total": total,
        "page": page,
        "limit": limit,
...
@router.post("/images/presign", summary="獲取 S3 預簽名 URL (管理員)")
async def get_image_presigned_url(
    data: ImagePresignRequest, admin: UserEntity = Depends(require_admin)
) -> dict:
    """獲取上傳圖片用的預簽名 URL"""
    ...
    upload_url = s3_client.generate_presigned_url(
        object_name=object_name, content_type=data.content_type
    )
...
async def admin_create_product(
    ...
    """建立新商品，僅限管理員權限"""
    try:
        from modules.product.infrastructure.repository import (
...
@router.put(
    "/{product_id}", response_model=ProductResponseDTO, summary="更新商品 (管理員)"
)
...
@router.delete(
    "/{product_id}", status_code=status.HTTP_204_NO_CONTENT, summary="刪除商品 (管理員)"
)
