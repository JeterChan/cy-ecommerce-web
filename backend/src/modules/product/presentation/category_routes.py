...
@router.get(
    "", response_model=List[CategoryResponseDTO], summary="取得所有分類 (管理員)"
)
async def admin_list_categories(
    db: AsyncSession = Depends(get_db), admin: UserEntity = Depends(require_admin)
...
async def admin_create_category(
    ...
    try:
        ...
        if await repo.get_by_slug(data.slug):
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT, detail="分類 Slug 已存在"
            )
        if await repo.get_by_name(data.name):
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT, detail="分類名稱已存在"
            )
        ...
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT, detail="分類名稱或 slug 已存在"
        )


@router.patch(
    "/{category_id}", response_model=CategoryResponseDTO, summary="更新分類 (管理員)"
)
async def admin_update_category(
    ...
    try:
        ...
        if not existing:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="分類不存在"
            )
