...
from modules.order.infrastructure.repositories.redis_cart_repository import (
    OrderCartAdapter,
)
...
async def list_orders(
    ...
    return OrderListResponse(orders=response_list, total=total, skip=skip, limit=limit)
...
async def update_order_status(
    ...
    try:
        user_uuid = (
            user.id if isinstance(user.id, uuid.UUID) else uuid.UUID(str(user.id))
        )
        order = await use_case.execute(
            order_id=order_id, user_id=user_uuid, new_status=request.status
        )
        return OrderResponse.model_validate(order)
    except ValueError as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
