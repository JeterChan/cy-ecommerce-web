...
def upgrade() -> None:
    ...
    op.create_table(
        "carts",
        sa.Column(
            "user_id", sa.UUID(), nullable=True, comment="會員 ID (會員購物車使用)"
        ),
        sa.Column(
            "guest_token",
    ...
    op.create_index(
        op.f("ix_carts_guest_token"), "carts", ["guest_token"], unique=False
    )
    op.create_index(
    ...
    op.create_table(
        ...
        sa.Column(
            "quantity", sa.Integer(), nullable=False, comment="購買數量 (必須 > 0)"
        ),
        sa.Column("id", sa.Uuid(), nullable=False),
        sa.Column(
            "created_at",
