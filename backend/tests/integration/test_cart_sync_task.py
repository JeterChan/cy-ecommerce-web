...
class TestCartSyncTaskIntegration:
    ...
    async def test_hybrid_repo_triggers_celery_task(
        self, async_session: AsyncSession, redis_client: Redis
    ):
        ...
        with patch(
            "modules.cart.infrastructure.tasks.sync_member_cart_task.apply_async"
        ) as mock_apply:
            # Act
    ...
    async def test_sync_task_persists_to_postgresql(
        self, async_session: AsyncSession, redis_client: Redis
    ):
        ...
        async with async_session.begin():
            result = await async_session.execute(
                text(
                    "SELECT ci.quantity FROM cart_items ci JOIN carts c ON ci.cart_id = c.id WHERE c.user_id = :uid"
                ),
    ...
    async def test_sync_task_handles_deletions(
        self, async_session: AsyncSession, redis_client: Redis
    ):
    ...
    async def test_sync_task_retries_on_lock_failure(self):
        ...
        user_id = str(uuid.uuid4())
        mock_task = MagicMock()
        mock_task.retry.side_effect = Exception(
            "Retry triggered"
