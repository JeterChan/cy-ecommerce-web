...
async def test_profile_update_and_password_change_flow(async_session):
    ...
    with pytest.raises(ValidationError) as exc:
        await update_use_case.execute(user.id, UpdateProfileRequest(username="taken_name"))
    ...
    # 4. 以新密碼登入 -> 應成功
    login_output = await login_use_case.execute(LoginRequestDTO(email=email, password=new_password))
