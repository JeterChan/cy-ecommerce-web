...
async def test_password_reset_flow_integration(async_session, redis_client):
    ...
    with patch("src.modules.auth.use_cases.register.send_registration_verification.delay"), \
         patch("src.modules.auth.application.use_cases.forgot_password.send_password_reset.delay") as mock_reset_task:
        ...
        reg_input = RegisterRequestDTO(username=username, email=email, password=old_password)
