...
async def test_verification_flow_integration(async_session, redis_client):
    ...
    with patch("modules.auth.use_cases.register.send_registration_verification.delay") as mock_email_task:
        ...
        # 1. 註冊
        reg_input = RegisterRequestDTO(username=username, email=email, password=password)
