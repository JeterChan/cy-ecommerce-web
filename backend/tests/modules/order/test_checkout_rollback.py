...
async def test_checkout_rollback_on_insufficient_stock(
    ...
    product1 = ProductModel(
        name="Valid Product", price=100.0, stock_quantity=10, is_active=True
    )
    product2 = ProductModel(
        name="Out of Stock Product", price=200.0, stock_quantity=0, is_active=True
    )
    ...
    # 7. 驗證訂單未建立
    res_orders = await async_db.execute(
