...
TEST_DATABASE_URL = os.getenv("TEST_DATABASE_URL", "postgresql+asyncpg://jeter:Oyn0BkxSj5TLxP@db:5432/test_ecommerce_db")
...
@pytest.mark.asyncio
async def test_checkout_rollback_on_insufficient_stock(
    ...
    # 記下 ID，避免後續存取 model 屬性觸發 lazy load
    ...
    await cart_repo.add_item(str(user_id), p1_id, 1)
    await cart_repo.add_item(str(user_id), p2_id, 1)
    ...
    response = await async_client.post("/api/v1/orders/checkout", json=checkout_data, headers=headers)
    ...
    # 6. 驗證原子性：商品 1 的庫存應未被扣除 (Rollback)
    res1 = await async_db.execute(select(ProductModel).where(ProductModel.id == p1_id))
    p1_after = res1.scalar_one()
    assert p1_after.stock_quantity == 10 
    
    # 7. 驗證訂單未建立
    res_orders = await async_db.execute(select(OrderModel).where(OrderModel.user_id == user_id))
    orders = res_orders.scalars().all()
    assert len(orders) == 0
    
    # 8. 驗證購物車未被清空
    cart_after = await cart_repo.get_cart(str(user_id))
    assert len(cart_after) == 2
