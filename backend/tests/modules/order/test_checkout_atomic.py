...
async def test_checkout_success(
    ...
    res1 = await async_db.execute(select(ProductModel).where(ProductModel.id == product1.id))
    p1_after = res1.scalar_one()
    assert p1_after.stock_quantity == 8
    
    res2 = await async_db.execute(select(ProductModel).where(ProductModel.id == product2.id))
